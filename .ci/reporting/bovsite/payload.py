...
def _build_payload(workspace: DealWorkspace) -> dict:
    ...
    for comp in sale["rows"]:
        ...
        # The rate environment at the sale (Glen, 2026-09-17): 3Y and 5Y
        # Treasury on the lock date, 30 days before the sale. Added only when
        # the spine carries it, so a 1.0.0 spine renders byte for byte as before.
        rate = comp.get("rate_at_sale")
        if rate and comp["status"] == "closed":
            rendered["treasury_3y"] = rate["treasury_3y_pct"]
            rendered["treasury_5y"] = rate["treasury_5y_pct"]
            rendered["rate_lock_date"] = rate["lock_date"]
    ...
    property_payload = {
        ...
        **({"rate_environment": deepcopy(sale["rate_environment"])}
           if sale.get("rate_environment") else {}),
