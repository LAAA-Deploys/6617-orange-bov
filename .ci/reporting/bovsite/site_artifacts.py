...
#: Written beside CNAME on every non-draft build. Search crawlers are ALLOWED
#: to fetch so they read the noindex meta tag and drop the page; a crawler
#: blocked here never sees that tag and can still index the bare URL from any
#: link to it. Training and scraping crawlers ignore noindex, so they are
#: refused outright.
ROBOTS_BODY = """# Generated by build_bov_site.py. Do not hand-edit.
User-agent: *
Allow: /
